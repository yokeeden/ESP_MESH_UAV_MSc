/**********************************************************************
* - Description:		ESP_MESH_BASE_NODE - main source file
* - File:				mesh_main.c
...
/*structure for received data storage*/
typedef struct {
    double pressure;
    double temperature;
    double humidity;
...
StackType_t xStack_MPU6050Task[ STACK_SIZE_4096 ];
...
static bool is_running = true; //task running flag
...
{
      /* status */
      esp_err_t status; //declaration of error handling variable

      /* routing table - to config mesh route table size open sdkconfig example configuration*/
      ...
      /*wait for INITIALIZATIONS_DONE bit */
    	  ...
    	  if(is_mesh_connected == true){
    		  ...
    		  /*if node is not a root node then receive data from measurement_task and send it wirelessly to root node */
    			  ...
    			  if( xStatus == pdPASS ){
    				  ...
    				  if (status == ESP_OK){
    					  ESP_LOGI(MESH_TAG, "[TX TASK] Sending measurement data message to root node: ("MACSTR")\n", MAC2STR(mesh_root_addr.addr));
    				  }
    				  /*error handling */
...
/**
  * @brief  MESH RX task - in case of base node receive data from connected sensor nodes and print it
  * @param  None
  * @retval None
  */
void esp_mesh_p2p_rx_main(void *arg)
{
	  ...
	  while (is_running) {
		  ...
		  if (esp_mesh_is_root()){
			  ...
			  /*print received data */
			  ESP_LOGI(MESH_TAG, "[RX TASK] Received data: Pressure: %.2f Pa, Temperature: %.2f C, Humidity: %.2f, Analog Temperature: %.1f  from node ("MACSTR")\n",
		  ...
		  /*if node is sensor node then it do not receive data  */
	  		  ...
	  		  ESP_LOGI(MESH_TAG,"[RX TASK] I am sensor node, so i do not recive any messages. I am a child to node: "MACSTR"\n",MAC2STR(mesh_parent_addr.addr));
	  		  vTaskDelay(10*1000 / portTICK_RATE_MS);
	  	  }
	  }
	  vTaskDelete(NULL);
...
esp_err_t esp_mesh_comm_p2p_start(void)
{
    ...
    if (!is_comm_p2p_started) {
        ...
        xTaskCreate(esp_mesh_p2p_tx_main, 	/* Function that implements the task. */
        									"MPTX", 	/* Text name for the task. */
											STACK_SIZE_3072, 		/* Stack depth */
											NULL, 		/* Parameter passed into the task. */
											8, 			/* Priority at which the task is created. */
											NULL); 		/* Pointer to task handle function. */

        xTaskCreate(esp_mesh_p2p_rx_main, 	/* Function that implements the task. */
        									"MPRX", 	/* Text name for the task. */
											STACK_SIZE_3072, 		/* Stack depth */
											NULL, 		/* Parameter passed into the task. */
											7, 			/* Priority at which the task is created. */
											NULL); 		/* Pointer to task handle function. */

        /*Static means that storage memory needs to be declared for task stack, check Enable Task Static Allocation API in sdkconfig*/
        ...
        xTaskCreateStaticPinnedToCore(measurement_task,        /* Function that implements the task. */
        	                                  "measurement_task",      /* Text name for the task. */
											  STACK_SIZE_4096,  /* Number of indexes in the xStack array. */
        	                                  NULL,                /* Parameter passed into the task. */
                        ...
                        int32_t event_id, void *event_data)
{
    ...
    case MESH_EVENT_PARENT_CONNECTED: {
        ...
        //set is_mesh_connected flag
        is_mesh_connected = true;
        //if a node is root node then start DHCP client
        if (esp_mesh_is_root()) {
...
void app_main(void)
{
	...
	ESP_ERROR_CHECK(nvs_flash_init()); /*init default NVS partition*/


    ESP_ERROR_CHECK(esp_netif_init());/*  tcp/ip stack initialization*/


    ESP_ERROR_CHECK(esp_event_loop_create_default());/*  event loop initialization*/
    ...
    wifi_init_config_t config = WIFI_INIT_CONFIG_DEFAULT();/*  wifi default initialization */
    ...
    /*    range 1 25 if MESH_TOPO_TREE
        ...
        default 6 to config CONFIG_MESH_MAX_LAYER open sdkconfig example configuration*/
